...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "text/*", "application/json", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/vnd.openxmlformats-officedocument.presentationml.presentation"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Upload for reports" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'reports');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for reports" ON storage.objects FOR UPDATE USING (bucket_id = 'reports');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for reports" ON storage.objects FOR DELETE USING (bucket_id = 'reports');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'reports',
        public: true,
        allowed_mime_types: ["application/pdf", "text/*", "application/json", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/vnd.openxmlformats-officedocument.presentationml.presentation"],
