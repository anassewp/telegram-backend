...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "video/*", "audio/*", "application/pdf", "text/*", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for campaign-files" ON storage.objects FOR SELECT USING (bucket_id = 'campaign-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for campaign-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'campaign-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for campaign-files" ON storage.objects FOR UPDATE USING (bucket_id = 'campaign-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for campaign-files" ON storage.objects FOR DELETE USING (bucket_id = 'campaign-files');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'campaign-files',
        public: true,
        allowed_mime_types: ["image/*", "video/*", "audio/*", "application/pdf", "text/*", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"],
